...
const float SimDTMF2::RINGBACK_FREQ_A = 440.0f;  // 440 Hz for ringback tone
const float SimDTMF2::RINGBACK_FREQ_C = 480.0f;  // 480 Hz for ringback tone  
const float SimDTMF2::BUSY_FREQ_A = 480.0f;      // 480 Hz for busy/reorder signals
const float SimDTMF2::BUSY_FREQ_C = 620.0f;      // 620 Hz for busy/reorder signals
const float SimDTMF2::DIAL_FREQ_A = 350.0f;      // 350 Hz for dial tone
const float SimDTMF2::DIAL_FREQ_C = 440.0f;      // 440 Hz for dial tone

// Legacy aliases for backward compatibility
const float SimDTMF2::RING_FREQ_A = SimDTMF2::RINGBACK_FREQ_A;
const float SimDTMF2::RING_FREQ_C = SimDTMF2::RINGBACK_FREQ_C;

// mode is expected to be a derivative of VFO
SimDTMF2::SimDTMF2(WaveGenPool *wave_gen_pool, SignalMeter *signal_meter, float fixed_freq, TelcoType type)
    : SimDualTone(wave_gen_pool, fixed_freq), _signal_meter(signal_meter), _telco_type(type)
{
    ...
    // Configure AsyncDTMF2 timing based on telco type
...
bool SimDTMF2::begin(unsigned long time){
    ...
    // Start AsyncDTMF2 ring cadence (repeating)
...
bool SimDTMF2::update(Mode *mode){
...
void SimDTMF2::set_retry_state(unsigned long next_try_time) {
    _in_wait_delay = true;
    _next_cycle_time = next_try_time;
}

void SimDTMF2::apply_operator_frustration_drift()
{
...
// Set frequency offsets based on telco type
void SimDTMF2::setFrequencyOffsetsForType() {
    switch (_telco_type) {
        case TELCO_RINGBACK:
...
float SimDTMF2::getFrequencyOffsetA() const {
    return _frequency_offset_a;
}

float SimDTMF2::getFrequencyOffsetC() const {
