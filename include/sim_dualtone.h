...
// Wave generator selection for dual generator mode
#define ENABLE_GENERATOR_A  // Enable by default
#define ENABLE_GENERATOR_C  // Enable for dual-tone telephony
...
// Common constants for dual-tone transmitters
...
/**
 * Base class for dual-tone telephony transmitters (DTMF, Ring, Busy, etc.)
 * Provides dual AD9833 wave generator support for authentic telephony sounds.
 *
 * INHERITANCE CONTRACT: All concrete telephony classes MUST inherit from both:
 ...
 * - Tone offsets are specified by derived classes for their specific telephony type
 *
...
public:
    SimDualTone(WaveGenPool *wave_gen_pool, float fixed_freq = 0.0, 
                float tone_offset_a = 0.0, float tone_offset_b = 0.0);
    ...
    virtual void randomize();  // Re-randomize station properties - default implementation does nothing
    ...
    // Dual-tone specific methods
    void set_tone_offsets(float tone_offset_a, float tone_offset_b);  // Set the telephony tone offsets
    void get_tone_offsets(float &tone_offset_a, float &tone_offset_b) const;  // Get current tone offsets
...
protected:    // Common utility methods
    ...
    float _fixed_freq;  // Target frequency for this station (shared between A and C)
    ...
    // Telephony tone offsets (set by derived classes)
    float _tone_offset_a;  // First tone offset (Hz)
    float _tone_offset_b;  // Second tone offset (Hz)

#if defined(ENABLE_GENERATOR_A) && defined(ENABLE_GENERATOR_C)
    // Wave Generator A variables (primary tone)
    float _frequency;   // Current frequency difference from VFO + tone offset A
    
    // Wave Generator C variables (secondary tone)
    float _frequency_c;   // Current frequency difference from VFO + tone offset B
    
    // Dynamic station management state (shared)
    DualToneState _station_state;  // Current state in dynamic management system
#elif defined(ENABLE_GENERATOR_A)
    // Wave Generator A variables (single tone mode)
    float _frequency;   // Current frequency difference from VFO + tone offset A
    
    // Dynamic station management state
    DualToneState _station_state;  // Current state in dynamic management system
#elif defined(ENABLE_GENERATOR_C)
    // Wave Generator C variables (single tone mode)
    float _frequency_c;   // Current frequency difference from VFO + tone offset A
